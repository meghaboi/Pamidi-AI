...
import numpy as np
from .enums import VectorStoreType # Relative import
import logging
import os
...
class HybridVectorStore(VectorStore):
    ...
    def __init__(self, alpha: float = 0.5):
        ...
        from .rag_pipeline import HybridSearch # Relative import
...
class VectorStoreFactory:
    ...
    @staticmethod
    def create_store(store_name: VectorStoreType, **kwargs) -> 'VectorStore': # Added type hint for store_name
        """Create a vector store based on the store name"""
        if store_name == VectorStoreType.FAISS:
            # FAISSVectorStore is defined in this file
            return FAISSVectorStore()
        elif store_name == VectorStoreType.CHROMA:
            # ChromaVectorStore is defined in this file
            return ChromaVectorStore()
        elif store_name == VectorStoreType.MILVUS:
            # MilvusVectorStore is defined in this file
            return MilvusVectorStore()
        # QdrantVectorStore could be added here if it was copied and intended for use
        # elif store_name == VectorStoreType.QDRANT: # Assuming QDRANT enum value exists
        #     return QdrantVectorStore()
        elif store_name == VectorStoreType.HYBRID:
            alpha = kwargs.get('alpha', 0.5)
            # HybridVectorStore is defined in this file
